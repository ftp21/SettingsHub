...
class AndreadelProvider(api.SettingProvider):
	id = ID
	name = NAME
	description = DESCRIPTION

	CATALOG_MAX_AGE = 300  # secondi: evita di ri-interrogare la GitHub API ad ogni click
	...
	# --- Le due funzioni sotto fanno rete vera (fetch della GitHub Contents
	# API): la UI dell'hub le chiama sempre dentro api.runInThread, mai
	# direttamente sul thread GUI (vedi screens/main.py).
