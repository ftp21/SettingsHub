...
class ManutekProvider(api.SettingProvider):
	id = ID
	name = NAME
	description = DESCRIPTION

	CATALOG_MAX_AGE = 300  # secondi: evita di ri-scaricare l'elenco (150+ zone) ad ogni click
	...
	# --- Le due funzioni sotto fanno rete vera (fetch della pagina AutoIndex):
	# la UI dell'hub le chiama sempre dentro api.runInThread, mai direttamente
	# sul thread GUI (vedi screens/main.py).
	def getCategories(self):
		entries = self._ensureCatalog()
		return [
			api.SettingCategory(id=cid, name=catalog.categoryDisplayName(cid))
